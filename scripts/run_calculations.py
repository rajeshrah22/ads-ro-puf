...
def main():
    if len(sys.argv) < 3:
        print("usage: python3 calculate_frequencies.py <input-dir> <output-dir>")
        sys.exit(1)
    ...
    for i in range(len(input_files)):
        time, normalized, buckets = calculate_frequencies.get_buckets(os.path.join(input_dir, input_files[i]), volts[i])
        calculate_frequencies.save_plot_freq_domain(time, normalized, buckets, os.path.join(output_dir, output_files[i]))
